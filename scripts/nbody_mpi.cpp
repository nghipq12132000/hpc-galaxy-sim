...
struct Octree {
    ...
    // Xây dựng cây từ tất cả hạt toàn cục
    void build() {
        nodes.clear();
        int n = px.size();
        if (n == 0) return;
...
int main(int argc, char **argv) {
    ...
    for (int step = 0; step < steps; step++) {
        ...
        // 2. Tính toán gia tốc cục bộ cho phân hoạch hạt của rank hiện tại sử dụng Barnes-Hut O(N log N)
        Octree tree(global_x, global_y, global_z, global_mass);
        tree.build();
        ...
        for (int i = 0; i < n_local; i++) {
            ...
            tree.calculate_force(0, local_x[i], local_y[i], local_z[i], global_i, 0.5, EPSILON, G, acc_x, acc_y, acc_z);
